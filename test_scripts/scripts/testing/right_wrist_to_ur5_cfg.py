...
CFG = "configs/ur5_right_u2d2_xl330.yaml" if len(sys.argv)<2 else sys.argv[1]
WRIST_INDEX = 5  # 0-based: J6
...
def main():
    ...
    wrist_id = ids[WRIST_INDEX]
    wrist_off = float(offsets[WRIST_INDEX])
    wrist_sign = int(signs[WRIST_INDEX])

    print(f"[RIGHT wrist→UR6] port={port} id={wrist_id} ur={ur_ip} acc={acc} vel={vel}")
