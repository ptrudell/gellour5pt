    ...
    def handle(self, channel, msg):
        ...
        if self.verbose:
            ...
            if timestamp_changed:  # Only print when data actually updates
                ...
                print(
                    f"\r[RIGHT {valid_mark}] #{self.msg_count:5d} ({rate:5.1f}Hz) "
...
def main():
    parser = argparse.ArgumentParser(
        description="Receive GELLO RIGHT arm positions from ZCM channel"
    )
    parser.add_argument(
        "-c",
        "--channel",
        ...
        help="ZCM channel name (default: gello_positions_right)",
    )
    parser.add_argument(
