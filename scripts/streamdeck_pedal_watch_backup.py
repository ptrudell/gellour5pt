...
class FollowThread:
    ...
    def _send_gripper_command(self, side: str, position: float):
        ...
        try:
            # First, always write to JSON file for any process to pick up
            ...
            # Also try the dexgpt script if it exists
            ...
            if os.path.exists(script_path):
                ...
                # Debug output if command failed
                if result.returncode != 0 and self.config.debug.get("verbose", False):
                    print(
                        f"[gripper] Command failed: {result.stderr.decode() if result.stderr else 'Unknown error'}"
                    )
            else:
                # Script doesn't exist, but we already wrote the JSON file
                if self.config.debug.get("verbose", False):
                    print("[gripper] Using JSON file only - dexgpt script not found")

        except subprocess.TimeoutExpired:
            # Command took too long, but JSON file was already written
            if self.config.debug.get("verbose", False):
                print("[gripper] Command timeout, but JSON file written")
        except Exception as e:
            ...
            if self.left_robot and self.left_controller:
                ...
                if dxl_pos is not None and ur_pos is not None:
                    ...
                    if self.config.gripper_enabled and len(dxl_pos) > 6:
                        ...
                        # Debug output for gripper position
                        if not hasattr(self, "_left_gripper_debug_count"):
                            self._left_gripper_debug_count = 0
                        self._left_gripper_debug_count += 1
                        if (
                            self._left_gripper_debug_count % 125 == 0
                        ):  # Every second at 125Hz
                            print(
                                f"[DEBUG] LEFT gripper pos: {gripper_pos:.3f} rad (ID 7)"
                            )
            ...
            if self.right_robot and self.right_controller:
                ...
                if dxl_pos is not None and ur_pos is not None:
                    ...
                    if self.config.gripper_enabled and len(dxl_pos) > 6:
                        ...
                        # Debug output for gripper position
                        if not hasattr(self, "_right_gripper_debug_count"):
                            self._right_gripper_debug_count = 0
                        self._right_gripper_debug_count += 1
                        if (
                            self._right_gripper_debug_count % 125 == 0
                        ):  # Every second at 125Hz
                            print(
                                f"[DEBUG] RIGHT gripper pos: {gripper_pos:.3f} rad (ID 16)"
                            )
