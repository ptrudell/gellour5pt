#!/usr/bin/env python3
"""
Receive and display GELLO RIGHT arm positions from ZCM channel.

Default channel: gello_positions_right
...
class GelloRightReceiver:
    def __init__(self, channel: str, verbose: bool = False):
        ...
        print(f"[ZCM] Listening RIGHT arm on '{self.channel}'")
        print("-" * 60)
    ...
    def _on_msg(self, ch: str, msg: gello_positions_t) -> None:
        ...
        if self.verbose:
            print(f"\n[RIGHT #{self.msg_count}]{rate_str}")
            print(
                f"  timestamp: {msg.timestamp} µs  side: {msg.arm_side}  valid: {msg.is_valid}"
            )
            for i, (rad, deg) in enumerate(
                zip(msg.joint_positions, joints_deg), start=10
            ):
                print(f"  J{i}: {deg:8.2f}°  ({rad:8.4f} rad)")
            print(
                f"  J16 (gripper): {grip_deg:8.2f}°  ({float(msg.gripper_position):8.4f} rad)"
            )
        else:
            joints_str = " ".join(
                f"J{i + 10}:{d:6.1f}°" for i, d in enumerate(joints_deg)
            )
            print(
                f"\r[RIGHT {ok}]{rate_str} {joints_str}  J16:{grip_deg:6.1f}°  (#{self.msg_count})",
                end="",
                flush=True,
            )

    def run(self) -> None:
        print("\nReceiving GELLO RIGHT…  (Ctrl+C to stop)\n")
        if not self.verbose:
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Receive GELLO RIGHT arm positions from ZCM"
    )
    ap.add_argument(
        "-c",
        "--channel",
        default="gello_positions_right",
        help="ZCM channel (default: gello_positions_right)",
    )
    ap.add_argument("-v", "--verbose", action="store_true", help="Verbose output")
    args = ap.parse_args()

    GelloRightReceiver(channel=args.channel, verbose=args.verbose).run()
